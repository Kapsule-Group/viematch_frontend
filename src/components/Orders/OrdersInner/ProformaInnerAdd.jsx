...
import { createProforma } from '../../../actions/ordersActions';
import {
  getInvoiceOptions,
...
class OrdersInnerAdd extends Component {
  ...
  submitForm = (data) => {
    const { createProforma, history, formInfo } = this.props;
    const { file, tax_file, notes, vatPercent, discountPercent } = this.state;
    ...
    data.due_date && formData.append('due_date', moment(data.due_date).format('YYYY-MM-DD'));
    ...
    createProforma(formData).then((res) => {
    ...
    return (
      <form className="order_page_inner content_block" onSubmit={handleSubmit(this.submitForm)}>
        <div className="custom_title_wrapper">
          ...
          <div className="title_page">Add proforma</div>
          <div className="content_page">
            ...
            <FieldArray
              ...
              products={products}
              itemsArr={itemsArr}
              role={this.props && this.props.role && this.props.role}
